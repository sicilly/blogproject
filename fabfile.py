...
def deploy(c):
    ...
    project_root_path = '~/apps/blogproject/'
    ...
    # 安装依赖，迁移数据库，收集静态文件
    with c.cd(project_root_path):
        c.run('/usr/bin/pipenv install --deploy --ignore-pipfile')
        c.run('/usr/bin/pipenv run python manage.py migrate')
        c.run('/usr/bin/pipenv run python collectstatic --noinput')
